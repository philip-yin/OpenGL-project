...
{       
    if (enablelighting) {       
		...
		vec3 _normal = (transpose(inverse(modelview)) * vec4(mynormal, 0.0)).xyz;
		vec3 normal = normalize(_normal) ; 

		for(int i = 0; i < numLights; i++){
			vec3 direction, halfv;
			if(lightposn[i].w == 0){
				direction = normalize(lightposn[i].xyz);
				halfv = normalize (direction + eyedirn);
			}
			else{
			...
			vec4 col = ComputeLight(direction, lightcolor[i], normal, halfv, diffuse, specular, shininess);
			finalcolor += col;
		}
